...
export class Score extends Component {
    ...
    constructor(item, selector) {
        ...
        for (let i = 1; i <= this.item.score; i++) {
            new Button(this.item.id, 'slot.fas', `<i class="icon--score fas fa-star" title="${i}/5"></i>`, 'score');
        }
        for (let j = this.item.score + 1; j <= 5; j++) {
            new Button(this.item.id, 'slot.far', `<i class="icon--score far fa-star" title="${j}/5"></i>`, 'score');
        }
    }
    createTemplate() {
        let html = '';
